...
import com.bezkoder.spring.hibernate.manytomany.model.TutorialRes;
...
public class TagController {
  ...
  public TagController(TutorialRepository tutorialRepository, TagRepository tagRepository, ModelMapper modelMapper) {
  ...
  public ResponseEntity<List<TutorialRes>> getAllTutorialsByTagId(@PathVariable(value = "tagId") Long tagId) {
    if (!tagRepository.existsById(tagId)) {
      throw new ResourceNotFoundException("Not found Tag  with id = " + tagId);
    }
    List<TutorialRes> resp = new ArrayList<>();
    List<Tutorial> tutorials = tutorialRepository.findAll();
      tutorials.forEach(x -> {
      resp.add(modelMapper.map(x, (Type) TutorialRes.class));}
    );
